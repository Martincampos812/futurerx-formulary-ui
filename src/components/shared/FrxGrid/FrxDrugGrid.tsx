 ...
 */
const CustomSortableItem = SortableElement((props) => <tr {...props} />);
const CustomSortableContainer = SortableContainer((props) => (
  <tbody {...props} />
));
...
> {
  ...
  componentDidUpdate(previousProps, previousState) {
    ...
    if (this.props.data.length !== previousState.sortedRows.length) {
      console.log(
        "updating sorted rowws ",
        this.props.data,
        previousState.sortedRows
      );
      this.setState({ sortedRows: [...this.props.data] });
    }
  }

  isGridRowsSame = (oldRows: any[], newRows: any[]) => {
    ...
    for (let i = 0; i < oldRows.length; i++) {
      if (oldRows[i].key !== newRows[i].key) {
        return false;
      }
    }
    return true;
  ...
  */
  ...
  /*
  ==================================================================================================
   BEGINNING OF GRID COLUMN RESIZING & ROW SWAP RELATED METHODS
  ==================================================================================================
  */
  ...
  onSortEnd = ({ oldIndex, newIndex }) => {
    // debugger;
    console.log("row dragging ", oldIndex, newIndex);
    this.setState({ isRowsSorted: true }, () => {
      const data = this.state.isRowsSorted
        ? [...this.state.sortedRows]
        : this.state.sortedInfo && !this.state.filteredInfo
        ? [...this.state.sortedTable]
        : this.state.filteredInfo && !this.state.sortedInfo
        ...
        ? this.state.sortedTable
        : [...this.props.data];
      // console.log("data in sring fun ", data);
      if (oldIndex !== newIndex) {
        ...
        if (this.props.getSortedData) this.props.getSortedData(newData);
        this.setState({ sortedRows: newData });
      }
    });
  };
  ...
  DraggableBodyRow = ({ className, style, ...restProps }) => {
    ...
    // console.log("rest props ", restProps["data-row-key"]);
    const index = data.findIndex((x) => {
      return x.index === parseInt(restProps["data-row-key"]);
    });
    // console.log("index ", index);
    return <CustomSortableItem index={index} {...restProps} />;
  };
