...
function mapDispatchToProps(dispatch) {
  return {
    getHpmsReports: (a) => dispatch(postHpmsReports(a)),
  };
}
...
class HmpsSubmissionTable extends React.Component<any, any>  {
  state = {
    filter: Array(),
    quickFilter: Array(),
    sort_by: Array(),
  ...
  populateGridData() {
    ...
    let apiDetails = {};
    apiDetails["apiPart"] = "api/1/hpms-reports";
    ...
    this.props.getHpmsReports(apiDetails).then((json) => {
  ...
  componentDidMount() {
    ...
    // let apiDetails_1 = {};
    // apiDetails_1["apiPart"] = "api/1/formulary-versions-list";
    // apiDetails_1["pathParams"] = "/" +  this.props?.formulary?.id_base_formulary;

    // this.props.getVersionList(apiDetails_1).then((json) => {
    //   if (json?.payload?.data?.length>0){
    //     let obj = json.payload.data[json.payload.data.length-1];

    //     this.setState({versionList: json.payload.data,
    //       versionId: obj.id_formulary,
    //       effectiveDate: obj.effective_date
    //     });
    ...
    if (this.props.formulary) {
      ...
      apiDetails["messageBody"] = {
        ...
        "sort_order": ["asc"],
        "template_type": 1,
        "formulary_id": 3389,//this.props?.formulary?.id_formulary,
        base_formulary_id: this.props?.formulary?.id_base_formulary,
      }
      this.props.getHpmsReports(apiDetails).then((json) => {
  ...
  handleExport = async (key: any) => {
    ...
    if (key != null) {
      files = [];
      files.push(this.state.gridData[key - 1]["code_value"]);
    }
    if (files.length==0){
      showMessage("Please select at least one file to export.", "error");
      return;
    }
    apiDetails_1["apiPart"] = "api/1/hpms-formulary-file";
    apiDetails_1["pathParams"] = "/" + this.props?.formulary?.id_formulary;
    let isError=false;
    ...
    for (const key of files) {
      ...
      try {
        ...
        if (response.data) {
          let tmp_file = this.state.gridData.filter((obj) => obj.code_value == key);
          let fileName = tmp_file[0].hpms_submission_export_name.replace(' ', '_') + "_Export";
  ...
  onSelectedTableRowChanged = (selectedRowKeys) => {
    ...
    if (selectedRowKeys && selectedRowKeys.length > 0) {
      this.state.selectedFiles = selectedRowKeys.map((tierId) => {
        return this.state.gridData[tierId - 1]["code_value"];
      });
    }
  };
          ...
          <div className="hmps-container">
            <div className="bordered">
              <div className="header space-between pr-10">
                HPMS SUBMISSION AND SUPPLEMENTAL OR ALTERNATIVE MODAL FORMULARY FILES
                <span className="wip-msg"> WIP : This tab will be is visible for Medicare</span>
              ...
              <div className="hmps-wrapper">
                <FrxDrugGridContainer
                  ...
                  gridName=""
                  enableSettings
                  columns={setupHmpsColumns({ exportFile: this.handleExport })}
                  scroll={{ x: 2000, y: 377 }}
...
export default connect(mapStateToProps, mapDispatchToProps)(HmpsSubmissionTable);