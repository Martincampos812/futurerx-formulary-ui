...
class DrugDetailIBF extends React.Component<any, any> {
  ...
  saveClickHandler = () => {
    if (this.state.selectedDrugs && this.state.selectedDrugs.length > 0) {
      ...
      if (this.state.activeTabIndex === 0 || this.state.activeTabIndex === 1) {
        ...
        let triggerType = (this.state.activeTabIndex === 0) ? ibfConstants.TYPE_REPLACE : ibfConstants.TYPE_APPEND
        apiDetails["pathParams"] = this.props?.formulary_id + "/" + this.state.lobCode + "/" + triggerType;
  ...
  onClickTab = (selectedTabIndex: number) => {
    ...
    this.setState({ tabs, activeTabIndex, selectedDrugs: [], selectedRxcuids: [], selectedCriteria: [], isSelectAll: false, showGrid: false });
  };
