...
import { getDrugDetailsColumnLIS } from "../DrugGridColumn";
import FrxLoader from "../../../../../shared/FrxLoader/FrxLoader";
import { getDrugDetailsLISSummary, getDrugDetailsLISList, getLISCriteriaList, postRemoveLISDrug, postReplaceLISDrug } from "../../../../../../redux/slices/formulary/drugDetails/lis/lisActionCreation";
...
class DrugDetailLIS extends React.Component<any, any> {
  state: lisState = {
    ...
    lisRemoveColumn: [
      {
        title: "LIS Cost-Share Amount",
        dataIndex: "lis_cost_sharing_amount",
  ...
  saveClickHandler = () => {
    if (this.state.selectedDrugs && this.state.selectedDrugs.length > 0) {
      let apiDetails = {};
      apiDetails["apiPart"] = lisConstants.APPLY_LIS_DRUG;
      ...
      if (this.state.activeTabIndex === 0) {
        ...
        this.rpSavePayload.lis_cost_sharing_amount = this.state.lisAmount
        this.rpSavePayload.is_select_all = this.state.isSelectAll
        apiDetails["messageBody"] = this.rpSavePayload;

        apiDetails["pathParams"] = this.props?.formulary_id + "/" + this.state.lobCode + "/" + lisConstants.TYPE_REPLACE;
        ...
        apiDetails["pathParams"] = this.props?.formulary_id + "/" + this.state.lobCode + "/" + lisConstants.TYPE_REMOVE;
  ...
  onPageSize = (pageSize) => {
    this.setState({ selectedRxcuids: [] }, () => {
      ...
      this.getLISDrugsList({ limit: this.listPayload.limit });
    });
  };
  ...
  getLISCriteriaList = () => {
    let apiDetails = {};
    apiDetails["apiPart"] = lisConstants.GET_LIS_CRITERIA_LIST;
    ...
    this.props.getLISCriteriaList(apiDetails).then((json) => {
      ...
      this.setState({
        lisRemoveData: settingsRows,
        showGrid: false,
      });
    });
  }

  getLISDrugsList = ({index = 0, limit = 10, listPayload = {}, searchBody = {}} = {}) => {
    let apiDetails = {};
    apiDetails["apiPart"] = lisConstants.GET_LIS_FORMULARY_DRUGS;
    apiDetails["pathParams"] = this.props?.formulary_id + "/" + getLobCode(this.props.formulary_lob_id);
    apiDetails["keyVals"] = [
      { key: lisConstants.KEY_ENTITY_ID, value: this.props?.formulary_id },
      { key: lisConstants.KEY_INDEX, value: index },
      { key: lisConstants.KEY_LIMIT, value: limit },
    ];
    apiDetails["messageBody"] = {};
    ...
    this.props.getDrugDetailsLISList(apiDetails).then((json) => {
      ...
      var gridData = tmpData.map((el) => {
        ...
        // for preseelct items with selected LIS value
        if(this.state.activeTabIndex !== 2) {
          if((element.is_lis && element.is_lis === "Y") && (element.lis_cost_sharing_amount && +element.lis_cost_sharing_amount === this.state.lisAmount)) {
  ...
  onClickTab = (selectedTabIndex: number) => {
    ...
    if(activeTabIndex === 2) {
      this.setState({ selectedCriteria: [] }, () => this.getLISCriteriaList());
    }
  ...
  validateForm = () => {
    ...
    if(this.state.activeTabIndex === 0 || this.state.activeTabIndex === 1) {
      formValid = !(this.state.lisAmount < 0);
    } else if(this.state.activeTabIndex === 2) {
  ...
  settingFormApplyHandler = () => {
    if(this.validateForm()) {
      ...
      this.state.activeTabIndex === 2 ? showMessage("Please select lis", "info") : showMessage("Please add lis", "info");
    }
  };
  ...
  onApplyFilterHandler = (filters) => {
    ...
    if (filters && filter.length > 0) {
      ...
      this.getLISDrugsList({ listPayload: this.listPayload });
    }
  }
  ...
  onApplySortHandler = (key, order, sortedInfo) => {
    ...
    if (this.props.advancedSearchBody) {
      this.getLISDrugsList({ searchBody: this.props.advancedSearchBody });
    } else {
      this.getLISDrugsList();
    }
  };
  ...
  componentWillReceiveProps(nextProps) {    
    ...
    if (nextProps.advancedSearchBody && nextProps.populateGrid) {
      this.getLISDrugsList({ listPayload: this.listPayload, searchBody: nextProps.advancedSearchBody});
      let payload = {
  ...
  inputChangeHandler = (e) => {
    if(e.target.name === "lisAmount") {
      this.state.lisAmount = +e.target.value;
    }
  }
  ...
  render() {
    ...
    let columns = getDrugDetailsColumnLIS();
              ...
              {this.state.activeTabIndex === 2 ? (
                <Grid item xs={5}>
                  <div className="tier-grid-remove-container other-removed-sec">
                    <Table
                      columns={this.state.lisRemoveColumn}
                      dataSource={this.state.lisRemoveData}
                      pagination={false}
                ...
                <Grid container>
                  <Grid item xs={6}>
                    <div className="group">
                      ...
                      <input type="number" name="lisAmount" onChange={(e) => this.inputChangeHandler(e)} />
                    </div>
                  </Grid>
                </Grid>
              )}
