...
class HpmsSummary extends Component<any, any> {
  state = {
    showCheckbox: false,
  ...
  getBackgroundColor = (type) => {
    switch (type) {
      case "Formulary File Enhancements":
        return "rgba(31, 187, 196, 0.4)";

      case "Negative Formulary File Changes":
        return "rgba(10, 123, 225, 0.4)";

      case "Non-Allowable Changes":
        return "#FFF5F0";
  ...
  populateSummaryData = async () => {
    if (this.props.formulary_lob_id && this.props.formulary_lob_id === 1) {
      let formularyTypesGridData = Array();
      let apiDetails = {};
      apiDetails["apiPart"] = "api/1/formulary-hpms-validations/";
      apiDetails["pathParams"] = this.props.formulary_id+"/"+this.props.formulary_id;
      ...
      try {
        ...
        if (data && data.length > 0) {
          ...
          data.map((value) => {
            let header = {
              id: idCount,
              title: value["validation_name"],
              titleBG: this.getBackgroundColor(value["validation_name"]),
              attribute_type: value["validation_name"],
              file_type: value["file_type"],
              gridColumns: ['Formulary File Enhancements', 'Negative Formulary File Changes', 'Non-Allowable Changes'],
              //.includes(value["validation_name"]) ? this.getGridColumns(value["validation_name"]) : Array(),
              headDrugsCount: {
                baseFormulary: null,
              },
              formularies: Array(),
            };
            //if (!this.areAllNull(value["validation_name"])) {
            ...
            if (value["values"] && value["values"].length > 0) {
              value["values"].map((subValue) => {
                ...
                let subItem = {
                  name: subValue["validation_name"],
                  baseFormulary: subValue["value"],
                  hoverDisplay:  subValue["hover_display"],
                  codeValue: subValue["code_value"],
                  idHpmsValidation: subValue["id_hpms_validation"],
...
export default connect(mapStateToProps, mapDispatchToProps)(HpmsSummary);
