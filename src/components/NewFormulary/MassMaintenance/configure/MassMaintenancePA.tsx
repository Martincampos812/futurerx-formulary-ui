  ...
  handleSave = () => {
    ...
    if (this.state.miniTabs[this.state.activeMiniTabIndex].value == "remove") {
      ...
      let cur_gd = Object.keys(this.state.selectedGDs).forEach(element => {
        ...
        if (tmp_gd.selectedPaType ==null){
          showMessage("Select the PA Type","info");
          isError=true;
        }
      ...
      this.props.maintenanceFormularies.list.map((curFormulary) => {
        ...
        if (that.state.miniTabs[that.state.activeMiniTabIndex].value == "remove") {
          ...
          if (cur_gd != null) {
            ...
            ] = cur_gd["selectedGroupDescription"];
            apiDetails["messageBody"][
              "id_pa_group_description"
            ] = cur_gd["selectedLastestedVersion"];
            apiDetails["messageBody"]["id_pa_type"] = Number(
              cur_gd["selectedPaType"]
            );
            if (
        ...
        that.props.postApplyFormularyDrugPA(apiDetails)
          .then((json) => {
      ...
      case 0:
        return <>{this.props.maintenanceFormularies.list.map((drug) => (
          <div className="mm-configure-pa-auth-grid-item">
            ...
            <PaReplace tab_type="replace" formularyId={drug.id_formulary} updateGDHandler={this.setGDForDrug} />
          </div>
        ))}</>;
        break;
      case 1:
          ...
          case 4:
            return <>{this.props.maintenanceFormularies.list.map((drug) => (
              <div className="mm-configure-pa-auth-grid-item">
                ...
                <PaReplace tab_type="append" formularyId={drug.id_formulary} updateGDHandler={this.setGDForDrug} />
              </div>
            ))}</>;
            break;
          default:
      ...
      case 2:
        return <>{this.props.maintenanceFormularies.list.map((drug) => (
          <div className="mm-configure-pa-auth-grid-item">
            ...
            <PaRemove tab_type="remove" formularyId={drug.id_formulary} updateGDHandler={this.setGDForRemove} />
          </div>
        ))}</>;
        break;
    }
