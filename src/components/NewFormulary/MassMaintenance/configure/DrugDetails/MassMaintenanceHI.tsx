...
class MassMaintenanceHI extends React.Component<any, any> {
  state = {
        ...
        return (
          <>
            {formularyList.map((drug) => (
              <MMHIReplace key={drug.id_formulary} drug={drug} />
            ))}
          </>
        );
      case 2:
        return (
          <>
            {formularyList.map((drug) => (
              <MMHIRemove key={drug.id_formulary} drug={drug} />
            ))}
          </>
        );
    }
  ...
  handleSave = () => {
    ...
    if (SELECTED_DRUGS && SELECTED_DRUGS.length > 0) {
      let apiDetails = {};
      apiDetails["apiPart"] = APPLY_HI_DRUG;
...
export default connect(mapStateToProps, mapDispatchToProps)(MassMaintenanceHI);

export const MMHIReplace = (props) => {
  const { drug } = props;
  ...
  return (
    <>
      <div className="mm-configure-pa-auth-grid-item" key={drug.id_formulary}>
        ...
        {checkIfTABApplicableByCode(drug, "HI") ? (
          <p>
