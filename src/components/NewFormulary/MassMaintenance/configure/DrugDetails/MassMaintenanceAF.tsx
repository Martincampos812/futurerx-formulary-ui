...
class MassMaintenanceAF extends React.Component<any, any> {
  state = {
        ...
        return (
          <>
            {formularyList.map((drug) => (
              <MMAFReplace key={drug.id_formulary} drug={drug} />
            ))}
          </>
        );
      case 2:
        return (
          <>
            {formularyList.map((drug) => (
              <MMAFRemove key={drug.id_formulary} drug={drug} />
            ))}
          </>
        );
    }
  ...
  handleSave = () => {
    ...
    if (SELECTED_DRUGS && SELECTED_DRUGS.length > 0) {
      let apiDetails = {};
      apiDetails["apiPart"] = APPLY_AF_DRUG;
...
export default connect(mapStateToProps, mapDispatchToProps)(MassMaintenanceAF);

export const MMAFReplace = (props) => {
  const { drug } = props;
