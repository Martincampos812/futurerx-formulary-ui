...
import FormularyExpandedDetails from "../DrugDetails/SelectFormularyPopUp/FormularyExpandedDetails";
import {
...
class MassMaintenanceFormularyPopup extends React.Component<any, any> {
  state = {
    formularyData: Array(),
  ...
  componentWillReceiveProps(nextProps){
    debugger;
    if (nextProps.maintenanceFormularies){
      let formularyGridData:any=[];
      let formularyData:any=[];
      let count=nextProps.maintenanceFormularies.count;
      let index=0;
      nextProps.maintenanceFormularies.list.map((row) => {
        let item = Object.assign({}, row);
        formularyData.push(item);
          let gridItem = {};
          gridItem["id"] = index + 1;
